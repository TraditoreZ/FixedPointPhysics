...
namespace TrueSync
{
    public class AABBOctreeNode<T>
    {
        // Centre of this node
        public TSVector Center { get; private set; }
        ...
        // Child nodes, if any
        AABBOctreeNode<T>[] children = null;

        bool HasChildren { get { return children != null; } }

        // AABB of potential children to this node. These are actual size (with looseness taken into account), not base size
        AABB[] childBounds;
        ...
        public AABBOctreeNode(FP baseLengthVal, FP minSizeVal, FP loosenessVal, TSVector centerVal)
        {
        ...
        public bool Add(T obj, AABB objBounds)
        {
        ...
        /// </summary>
        /// <param name="checkBounds">AABB to check.</param>
        /// <returns>True if there was a collision.</returns>
        public bool IsColliding(ref AABB checkBounds)
        {
            ...
            for (int i = 0; i < objects.Count; i++)
            {
                if (objects[i].AABB.Intersects(checkBounds))
                {
                    return true;
                }
            }
        ...
        /// </summary>
        /// <param name="checkBounds">AABB to check. Passing by ref as it improves performance with structs.</param>
        /// <param name="result">List result.</param>
        /// <returns>Objects that intersect with the specified bounds.</returns>
        public void GetColliding(ref AABB checkBounds, List<T> result)
        {
            // Are the input bounds at least partially in this node?
            if (!bounds.Intersects(checkBounds))
            {
        ...
        public void SetChildren(AABBOctreeNode<T>[] childOctrees)
        {
        ...
        public void DrawAllBounds(float depth = 0)
        {
            ...
            AABB thisBounds = new AABB(Center, new TSVector(adjLength, adjLength, adjLength));
        ...
        {
            float tintVal = BaseLength.AsFloat() / 20;
            Gizmos.color = new Color(0, 1.0f - tintVal, tintVal, 0.25f);

            foreach (OctreeObject obj in objects)
            {
                Gizmos.DrawCube(obj.AABB.center.ToVector3(), obj.AABB.size.ToVector3());
            }
        ...
        public AABBOctreeNode<T> ShrinkIfPossible(FP minLength)
        {
            ...
            for (int i = 0; i < objects.Count; i++)
            {
                ...
                int newBestFit = BestFitChild(curObj.AABB.center);
                if (i == 0 || newBestFit == bestFit)
                {
                    // In same octant as the other(s). Does it fit completely inside that octant?
                    if (Encapsulates(childBounds[newBestFit], curObj.AABB))
                    {
                        if (bestFit < 0)
                        {
                            bestFit = newBestFit;
        ...
        public int BestFitChild(TSVector objBoundsCenter)
        {
        ...
        */
        ...
        /// <param name="loosenessVal">Multiplier for baseLengthVal to get the actual size.</param>
        /// <param name="centerVal">Centre position of this node.</param>
        void SetValues(FP baseLengthVal, FP minSizeVal, FP loosenessVal, TSVector centerVal)
        {
            ...
            TSVector size = new TSVector(adjLength, adjLength, adjLength);
            bounds = new AABB(Center, size);

            FP quarter = BaseLength / 4f;
            FP childActualLength = (BaseLength / 2) * looseness;
            TSVector childActualSize = new TSVector(childActualLength, childActualLength, childActualLength);
            childBounds = new AABB[8];
            childBounds[0] = new AABB(Center + new TSVector(-quarter, quarter, -quarter), childActualSize);
            childBounds[1] = new AABB(Center + new TSVector(quarter, quarter, -quarter), childActualSize);
            childBounds[2] = new AABB(Center + new TSVector(-quarter, quarter, quarter), childActualSize);
            childBounds[3] = new AABB(Center + new TSVector(quarter, quarter, quarter), childActualSize);
            childBounds[4] = new AABB(Center + new TSVector(-quarter, -quarter, -quarter), childActualSize);
            childBounds[5] = new AABB(Center + new TSVector(quarter, -quarter, -quarter), childActualSize);
            childBounds[6] = new AABB(Center + new TSVector(-quarter, -quarter, quarter), childActualSize);
            childBounds[7] = new AABB(Center + new TSVector(quarter, -quarter, quarter), childActualSize);
        }
        ...
        /// <param name="obj">Object to add.</param>
        /// <param name="objBounds">3D bounding box around the object.</param>
        void SubAdd(T obj, AABB objBounds)
        {
            ...
            if (!HasChildren)
            {
                ...
                {
                    OctreeObject newObj = new OctreeObject { Obj = obj, AABB = objBounds };
                    objects.Add(newObj);
                ...
                if (children == null)
                {
                    ...
                    for (int i = objects.Count - 1; i >= 0; i--)
                    {
                        ...
                        bestFitChild = BestFitChild(existingObj.AABB.center);
                        // Does it fit?
                        if (Encapsulates(children[bestFitChild].bounds, existingObj.AABB))
                        {
                            children[bestFitChild].SubAdd(existingObj.Obj, existingObj.AABB); // Go a level deeper					
        ...
        /// <summary>
        /// Private counterpart to the public <see cref="Remove(T, AABB)"/> method.
        /// </summary>
        /// <param name="obj">Object to remove.</param>
        /// <param name="objBounds">3D bounding box around the object.</param>
        /// <returns>True if the object was removed successfully.</returns>
        bool SubRemove(T obj, AABB objBounds)
        {
            bool removed = false;
        ...
        void Split()
        {
            ...
            children = new AABBOctreeNode<T>[8];
            children[0] = new AABBOctreeNode<T>(newLength, minSize, looseness, Center + new TSVector(-quarter, quarter, -quarter));
            children[1] = new AABBOctreeNode<T>(newLength, minSize, looseness, Center + new TSVector(quarter, quarter, -quarter));
            children[2] = new AABBOctreeNode<T>(newLength, minSize, looseness, Center + new TSVector(-quarter, quarter, quarter));
            children[3] = new AABBOctreeNode<T>(newLength, minSize, looseness, Center + new TSVector(quarter, quarter, quarter));
            children[4] = new AABBOctreeNode<T>(newLength, minSize, looseness, Center + new TSVector(-quarter, -quarter, -quarter));
            children[5] = new AABBOctreeNode<T>(newLength, minSize, looseness, Center + new TSVector(quarter, -quarter, -quarter));
            children[6] = new AABBOctreeNode<T>(newLength, minSize, looseness, Center + new TSVector(-quarter, -quarter, quarter));
            children[7] = new AABBOctreeNode<T>(newLength, minSize, looseness, Center + new TSVector(quarter, -quarter, quarter));
        }
        ...
        void Merge()
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                AABBOctreeNode<T> curChild = children[i];
        ...
        static bool Encapsulates(AABB outerBounds, AABB innerBounds)
        {
        ...
        bool ShouldMerge()
        {
            ...
            if (children != null)
            {
                foreach (AABBOctreeNode<T> child in children)
                {
